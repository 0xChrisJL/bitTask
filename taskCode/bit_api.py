...
payload_config = {
    ...
    "platform": '',  # 账号平台
    "platformIcon": 'others',  # 取账号平台的 hostname 或者设置为other
    "url": '',  # 打开的url，多个用,分开
    "name": '',  # 窗口名称
    # 备注
    "remark": '',
...
class BitBrowser:
    ...
    ##############################################浏览器接口#########################################################
    def create_driver(self, username='', password='', proxyType='noproxy', proxyIp='', proxyPort='', proxyUser='',
                      proxyPassword='', **kwargs):
        """
        创建浏览器
        仅传入用户名、密码、代理
        """
        ...
        fingerprint['version'] = random.randint(100, 106)
        ...
        fingerprint['openWidth'] = random.choice([1280, 1366])
        fingerprint['openHeight'] = random.choice([720, 800, 1080])
    ...
    def windowbounds(self, type: str = 'box', startX: int = 0, startY: int = 0, width: int = 500, height: int = 500,
                     col: int = 3, spaceX: int = 0, spaceY: int = 0, offsetX: int = 0, offsetY: int = 0):
        """
        窗口排列
    ...
    ###################################################自定义的一些方法########################################################
    def get_driver(self, username='', password='', proxyType='noproxy', proxyIp='', proxyPort='', proxyUsername='',
                   proxyPassword='', **kwargs):
        """
